...
for item in returned_data['oppHits']:
    # log file
    n += 1
    with open("temp_grants_gov_ca.log", "a") as log:
        log.write(
                "buscando %s (%d de %d)\n" %
    ...
    links.append(link)
    # with open("temp_grants_gov_ca.json", "a") as out:
    #     out.write(json.dumps(link)+"\n")


# Transform temp file into one complete json
# with open("temp_grants_gov_ca.json", "r") as f_in:
...
with open("grants_gov_ca.json", "w") as out:
